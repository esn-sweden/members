...
const invalidContainer =
  document.querySelector<HTMLDivElement>("#invalidContainer")!;
...
async function handleFileInput(e: Event) {
  ...
  try {
    ...
    rows = XLSX.utils
      .sheet_to_json<Record<string, any>>(sheet, { defval: "" })
      .slice(5);
  } catch (err) {
  ...
  if (rows.length === 0 || !("Personnummer" in rows[0])) {
    fileError.textContent =
      "Your file does not appear to have a personnummer column";
    fileInput.classList.add("is-invalid");
    return;
  } else {
    fileError.textContent = "";
...
function updateStats(persons: Person[]) {
  ...
  for (const pn of invalidPn) {
    const bullet = document.createElement("li");
    bullet.textContent = pn;
    invalidList.appendChild(bullet);
  }
